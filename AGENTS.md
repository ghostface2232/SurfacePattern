  ...
  libraries/
    surfacepattern/
      core/
        ...
        mapping.py               # UV<->3D, frames, distortion, pullback
...
- core/mapping.py owns every UV-to-world conversion, surface frame lookup, distortion compensation, world-grid projection, and curve pullback. No other module calls Surface.PointAt / FrameAt / PullToBrepFace.
...
- Placement modes. UV mode lays the pattern in a single face's normalized UV domain — precise, fast, but discontinuous across polysurface face borders. World-projection mode builds a lattice on a reference plane in 3D and projects each point to the closest face — continuous across faces, slight distortion depending on projection direction. Default: UV for a single face, world-projection for polysurfaces. Both must remain supported.
